...
def is_device_in_use(device):
    return (
        ...
        or redis_client.sismember(abandon_key, device["name"])
    )
        ...
        if device:
            ...
            if is_start:
                while True:
                    ...
                    logger.warn("Test exit with stdout: " + stdout)
                    logger.warn("Test exit with stderr: " + stderr)
                    if stdout.find("java.lang.RuntimeException: DeviceScrapped") >= 0:
                        redis_client.sadd(abandon_key, device["name"])
                        await stop_device(device)
                        break
                    elif (
                        ...
                        await stop_device(device)
                        fail_count = device_fail_count.pop(device["name"], 0)
                        if fail_count <= 2:
                            device_fail_count[device["name"]] = fail_count + 1
                        else:
                            redis_client.sadd(abandon_key, device["name"])
                        break
                    device_fail_count.pop(device["name"], None)
...
async def main():
    await asyncio.gather(*[task() for i in range(3)])
